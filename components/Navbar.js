    ...
    <div className='flex flex-col md:flex-row md:justify-start  justify-center items-center py-2 shadow-md'>
      ...
      <div ref={ref} className="w-72 h-full sideCart absolute top-0 right-0 bg-orange-100 px-8 py-10 transition-transform translate-x-full transform">
          ...
          {Object.keys(cart).map((k)=>{ return <li key={k}>
            <div className="item flex my-5">
              ...
              <div className='flex font-semibold items-center justify-center w-1/3 text-lg'>
                <AiFillPlusCircle className='cursor-pointer text-orange-500' /><span className='mx-2 text-sm'>{cart[k].qty}</span>
        ...
        <div className="flex">
          ...
          <button onClick={clearCart} className="flex mr-1 text-white bg-orange-500 border-0 py-2 px-2 focus:outline-none hover:bg-orange-600 rounded text-sm"><BsBagHeartFill className='m-1' /> Clear Cart</button>
        </div>
      </div>
    </div>
  )
